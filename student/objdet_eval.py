    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for det in detections:
                ...
                dist_x = np.abs(label.box.center_x-x)
                dist_y = np.abs(label.box.center_y-y)
                dist_z = np.abs(label.box.center_z-z)
...
def compute_performance_stats(det_performance_all):
    ...
    all_positives = sum(arr[0] for arr in pos_negs)
    true_positives = sum(arr[1] for arr in pos_negs)
    false_negatives = sum(arr[2] for arr in pos_negs)
    false_positives = sum(arr[3] for arr in pos_negs)
